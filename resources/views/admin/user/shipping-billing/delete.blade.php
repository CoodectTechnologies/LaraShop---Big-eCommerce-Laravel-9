<a onclick="event.preventDefault(); confirmDestroyBillingAddress({{ $billingAddress->id }})" class="btn btn-light-danger btn-active-primary my-1 me-2">{{ __('Delete') }}</a>
@once
    @push('footer')
        <script>
            function confirmDestroyBillingAddress(id){
                swal.fire({
